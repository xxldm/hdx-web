...
- Web 默认以桌面浏览器为主，不为手机 Web 单独适配布局或编辑交互。
- 手机端体验由后续 App 承接；手机浏览器不是当前首页工具箱的验收范围。
- 首页工具箱和编辑态交互继续统一基于 Pointer Events，实现鼠标与桌面宽度触摸输入共用一套事件模型。
- 关键操作不能只依赖 hover；触摸场景需要保留 tap 或显式按钮入口作为兜底。
...
登录页位于 `app/pages/login.vue`，采用轻量液态玻璃风格和纯 CSS 明暗双态背景，不依赖外部绝对路径或单一背景图资产。
...
```powershell
& '<python.exe>' .codex/skills/ui-ux-pro-max/scripts/search.py "HDX consumer personal toolbox liquid glass playful user app" --design-system --persist -p "HDX Web" --page "workbench" -f markdown
```

当前设计系统已生成到 `design-system/hdx-web/`。HDX Web 默认是偏用户端的个人工具箱风格，不是企业后台、SaaS 控制台或正式工作台。构建具体页面时，先读 `design-system/hdx-web/pages/<page>.md`，没有页面规则时再读 `design-system/hdx-web/MASTER.md`。
...
如果本机后端未启动，首屏应显示 Web 工具箱骨架和后端不可用提示；这不应阻塞 Web build。
